...
public class PreferredCreatureTypeDecision
{
    ...
    /// </summary>
    public CreatureSO Decide(CreatureSO excludeCreature = null)
    {
        var ranked = BuildRankedList(excludeCreature);
    ...
    // Only ever called after ShouldSummonCreaturesDecision returns true, so EnemyBoardFull is
    // guaranteed false here and ranked is guaranteed non-empty (rule 5 — no defensive handling needed).
    private static List<CreatureSO> BuildRankedList(CreatureSO excludeCreature)
    {

...
{
    /// <summary>
    /// Pure view for the Shock nuke. Takes the pre-computed shot list from
    /// <see cref="ShockResolver"/> and fires one <see cref="SimpleProjectile"/> per shot
    /// (LightningBall), applying the shocked status on impact. No targeting / status logic
    /// here — that's the resolver / <see cref="ShockShot"/>'s job.
    /// </summary>
    public class ShockAnimation : NukeActionAnimation
    {
        [Header("Cast Timing")]
        ...
        [Header("Projectile")]
        [Tooltip("Self-contained projectile prefab (LightningBall).")]
